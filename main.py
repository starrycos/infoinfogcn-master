...
import resource
rlimit = resource.getrlimit(resource.RLIMIT_NOFILE)  # rlimit包括软限制和硬限制
resource.setrlimit(resource.RLIMIT_NOFILE, (2048, rlimit[1]))  # 这里更改软限制为2048和硬限制不变
...
class Processor():
    ...
    def print_log(self, str, print_time=True):
        if print_time:
            localtime = time.asctime(time.locatimltime(time.time()))
            str = "[ " + localtime + ' ] ' + str
        print(str)
